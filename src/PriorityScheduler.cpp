...
std::function<void(size_t)> PriorityCallbackGroupHandle::get_ready_callback_for_entity(const rclcpp::SubscriptionBase::WeakPtr & entity)
{
...
std::function<void(std::function<void()> executed_callback)> PriorityCallbackGroupHandle::get_ready_callback_for_entity(const rclcpp::TimerBase::WeakPtr & entity)
{
...
//         RCUTILS_LOG_INFO_NAMED("FirstInFirstOutCallbackGroupHandle", "TimerBase ready callback called");
        ...
        ready_timers.emplace_back(ReadyEntity::ReadyTimerWithExecutedCallback{weak_ptr, executed_callback});

        check_move_to_ready();
    };
}

std::function<void(size_t)> PriorityCallbackGroupHandle::get_ready_callback_for_entity(const rclcpp::ClientBase::WeakPtr & entity)
{
    return [weak_ptr = entity, this](size_t nr_msg) {
        std::lock_guard l(ready_mutex);
...
std::function<void(size_t)> PriorityCallbackGroupHandle::get_ready_callback_for_entity(const rclcpp::ServiceBase::WeakPtr & entity)
{
    return [weak_ptr = entity, this](size_t nr_msg) {
        std::lock_guard l(ready_mutex);
...
std::function<void(size_t, int)> PriorityCallbackGroupHandle::get_ready_callback_for_entity(const rclcpp::Waitable::WeakPtr & entity)
{
...
//         RCUTILS_LOG_ERROR_NAMED("rclcpp", "Waitable got data");
        for (size_t i = 0; i < nr_msg; i++) {
            ready_waitables.emplace_back(CBGScheduler::WaitableWithEventType({weak_ptr, event_type}));
        }

        check_move_to_ready();
    };
}
std::function<void(size_t)> PriorityCallbackGroupHandle::get_ready_callback_for_entity(const CBGScheduler::CallbackEventType & entity)
{
    return [weak_ptr = entity, this](size_t nr_msg) {
        std::lock_guard l(ready_mutex);
...
std::optional<CBGScheduler::ExecutableEntity> PriorityCallbackGroupHandle::get_next_ready_entity(enum Priorities for_priority)
{
    switch(for_priority)
    {
        case Calls:
...
std::optional<CBGScheduler::ExecutableEntity> PriorityCallbackGroupHandle::get_next_ready_entity(GlobalEventIdProvider::MonotonicId max_id, enum Priorities for_priority)
{
    switch(for_priority)
    {
        case Calls:
...
std::optional<CBGScheduler::ExecutableEntity> PriorityCallbackGroupHandle::get_next_ready_entity(std::deque<ReadyEntity> &queue)
{
...
std::optional<CBGScheduler::ExecutableEntity> PriorityCallbackGroupHandle::get_next_ready_entity(GlobalEventIdProvider::MonotonicId max_id, std::deque<ReadyEntity> &queue)
{
    std::lock_guard l(ready_mutex);
...
std::unique_ptr<PriorityScheduler::CallbackGroupHandle> PriorityScheduler::get_handle_for_callback_group(const rclcpp::CallbackGroup::SharedPtr &/*callback_group*/)
{
...
std::optional<PriorityScheduler::ExecutableEntity> PriorityScheduler::get_next_ready_entity()
{
    ...
    {
        PriorityCallbackGroupHandle::Priorities cur_prio(static_cast<PriorityCallbackGroupHandle::Priorities>(
            i ) );
        ...
        for (auto it = ready_callback_groups.begin(); it != ready_callback_groups.end(); it++)
        {
            ...
            std::optional<PriorityScheduler::ExecutableEntity> ret = ready_cbg->get_next_ready_entity(cur_prio);
            if(ret)
            {
...
std::optional<PriorityScheduler::ExecutableEntity> PriorityScheduler::get_next_ready_entity(GlobalEventIdProvider::MonotonicId max_id)
{
    std::lock_guard l(ready_callback_groups_mutex);
        ...
        i <= PriorityCallbackGroupHandle::Priorities::Waitable; i++)
    {
        ...
        {
            PriorityCallbackGroupHandle *ready_cbg(static_cast<PriorityCallbackGroupHandle *>(*it));
            std::optional<PriorityScheduler::ExecutableEntity> ret = ready_cbg->get_next_ready_entity(max_id, cur_prio);
            if(ret)
            {
                ready_callback_groups.erase(it);
                return ret;
